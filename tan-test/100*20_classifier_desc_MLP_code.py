...
class FunctionClassifier(nn.Module):
    ...
    def forward(self, code_emb, desc_emb):
        ...
        # print(code_emb.shape)#torch.Size([32, 768])
        # print(desc_emb.shape)#torch.Size([32, 768])
        
        code_out = self.relu(self.fc1_code(code_emb))
        desc_out = self.relu(self.fc1_desc(desc_emb))
        # print(code_out.shape)#torch.Size([32, 256])
        # print(desc_out.shape)#torch.Size([32, 256])
        # 计算注意力得分
        code_attn_scores = self.attn(code_out)
        desc_attn_scores = self.attn(desc_out)
        # print(code_attn_scores.shape)#torch.Size([32,1])
        # print(desc_attn_scores.shape)#torch.Size([32,1])
        # 归一化注意力得分
        code_attn_weights = torch.softmax(code_attn_scores, dim=0)
        desc_attn_weights = torch.softmax(desc_attn_scores, dim=0)
        # print(code_attn_weights.shape)#torch.Size([32, 1])
        # print(desc_attn_weights.shape)#torch.Size([32, 1])
        # 加权求和得到上下文向量
        code_context = torch.sum(code_attn_weights.unsqueeze(-1) * code_out, dim=0)
        desc_context = torch.sum(desc_attn_weights.unsqueeze(-1) * desc_out, dim=0)
        # print(code_context.shape)#torch.Size([32,256])
        # print(desc_context.shape)#torch.Size([32,256])
        # 调整上下文向量的形状
        code_context = code_context.view(-1, 256)
        desc_context = desc_context.view(-1, 256)
        # print(code_context.shape)#torch.Size([32,256])
        # print(desc_context.shape)#torch.Size([32,256])
        
        combined = torch.cat((code_context, desc_context), dim=1)  # 拼接后的大小为 (batch_size, 512)
...
results = []
steps = np.arange(0.05, 1.00, 0.05)
for j in steps:
    # 定义结果存储列表
    accuracies = []
    precisions = []
    ...
    result = {
        ...
        'Total Confusion Matrix': total_confusion_matrix.tolist()
    }
