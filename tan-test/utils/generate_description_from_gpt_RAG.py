...
# 获取项目的根目录（假设项目根目录在 utils 文件夹的上两级）
project_root = os.path.abspath(os.path.join(current_dir, '../'))

# 将项目根目录添加到 sys.path 中
sys.path.append(project_root)

from get_report_from_url.rag_enhanced_llm import search_from_reports_embedding
...
commented_code_dir = os.path.abspath(os.path.join(script_dir, '..', 'functions_desc_0_RAG'))
...
def process_file(api_key, filename):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        """
        find_reports = search_from_reports_embedding(code, top_k=3)
        question = question_head +"[REFERENCE VULNERABILITY REPORTS]"+f"\n\n {find_reports}" + f"\n\n please do the job according to the code follows: [TASK CODE] \n{code}" 
...
def main():
    ...
    progress_bar.close()
    print(f"所有文件已经处理并保存到 {commented_code_dir} 文件夹中。")
